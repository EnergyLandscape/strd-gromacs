                             ...
                             gmx_bool bExtended, real veta)
{
    ...
    for (n = start; n < nrend; n++)
    {
        ...
        // ===================================================================
		//SRDCODE bounceback boundary conditions for top and bottom surfaces
		// This has not been fully implemented. When enabled, collision cells
		// split across the periodic boundary should be treated separately and
		// should include 'virtual' SRD particles in the part of the cells
		// that extend out into the void. See doi: 10.1088/0953-8984/22/10/104106
		/*
		if(typeA[n] == srd->type)
		{
			real H = state->box[ZZ][ZZ];
			real zi = x[n][ZZ];
			real zf = xprime[n][ZZ];
			...
			if(zf > H)
			{
				real s = (2 * (H - zi) / (zf - zi)) - 1;
				xprime[n][XX] = x[n][XX] + s * (xprime[n][XX] - x[n][XX]);
				xprime[n][YY] = x[n][YY] + s * (xprime[n][YY] - x[n][YY]);
				xprime[n][ZZ] = x[n][ZZ] + s * (xprime[n][ZZ] - x[n][ZZ]);
				v[n][XX] = -v[n][XX];
				v[n][YY] = -v[n][YY];
				v[n][ZZ] = -v[n][ZZ];
			}
