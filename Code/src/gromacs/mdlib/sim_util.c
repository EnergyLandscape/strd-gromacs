...
void do_force_cutsVERLET(FILE *fplog, t_commrec *cr,
                         ...
                         matrix box, rvec particlePositionArray[], history_t *hist,
                         rvec f[],
                         tensor vir_force,
                         t_mdatoms *mdatoms,
                         ...
                         int flags)
{
    ...
    if (bStateChanged)
    {
        ...
        if (NEED_MUTOT(*inputrec))
        {
            ...
            calc_mu(start, homenr,
                    particlePositionArray, mdatoms->chargeA, mdatoms->chargeB, mdatoms->nChargePerturbed,
                    mu, mu+DIM);
        }
    }
    ...
    if (fr->ePBC != epbcNONE)
    {
        ...
        if (bCalcCGCM)
        {
            put_atoms_in_box_omp(fr->ePBC, box, homenr, particlePositionArray);
            inc_nrnb(nrnb, eNR_SHIFTX, homenr);
        }
        else if (EI_ENERGY_MINIMIZATION(inputrec->eI) && graph)
        {
            unshift_self(graph, box, particlePositionArray);
        }
    }
    ...
    if (!(cr->duty & DUTY_PME))
    {
        ...
        gmx_pme_send_coordinates(cr, bBS ? boxs : box, particlePositionArray,
    ...
    if (bNS)
    {
        if (graph && bStateChanged)
        {
            ...
            mk_mshift(fplog, graph, fr->ePBC, box, particlePositionArray);
        }
        ...
        if (!fr->bDomDec)
        {
            ...
            nbnxn_put_on_grid(nbv->nbs, fr->ePBC, box,
                              0, vzero, box_diag,
                              0, mdatoms->homenr, -1, fr->cginfo, particlePositionArray,
                              0, NULL,
        ...
        else
        {
            ...
            nbnxn_put_on_grid_nonlocal(nbv->nbs, domdec_zones(cr->dd),
                                       fr->cginfo, particlePositionArray,
    ...
    else
    {
        ...
        nbnxn_atomdata_copy_x_to_nbat_x(nbv->nbs, eatLocal, FALSE, particlePositionArray,
                                        nbv->grp[eintLocal].nbat);
    ...
    if (DOMAINDECOMP(cr))
    {
        ...
        if (bDiffKernels)
        {
            ...
            wallcycle_start(wcycle, ewcNB_XF_BUF_OPS);
            wallcycle_sub_start(wcycle, ewcsNB_X_BUF_OPS);
            nbnxn_atomdata_copy_x_to_nbat_x(nbv->nbs, eatLocal, TRUE, particlePositionArray,
                                            nbv->grp[eintNonlocal].nbat);
            wallcycle_sub_stop(wcycle, ewcsNB_X_BUF_OPS);
            wallcycle_stop(wcycle, ewcNB_XF_BUF_OPS);
        }
        ...
        else
        {
            ...
            dd_move_x(cr->dd, box, particlePositionArray);
            ...
            wallcycle_start(wcycle, ewcNB_XF_BUF_OPS);
            wallcycle_sub_start(wcycle, ewcsNB_X_BUF_OPS);
            nbnxn_atomdata_copy_x_to_nbat_x(nbv->nbs, eatNonlocal, FALSE, particlePositionArray,
                                            nbv->grp[eintNonlocal].nbat);
            wallcycle_sub_stop(wcycle, ewcsNB_X_BUF_OPS);
    ...
    if (inputrec->bRot)
    {
        ...
        do_rotation(cr, inputrec, box, particlePositionArray, t, step, wcycle, bNS);
    ...
    if (fr->efep != efepNO)
    {
        ...
        if (fr->nbv->grp[eintLocal].nbl_lists.nbl_fep[0]->nrj > 0)
        {
            do_nb_verlet_fep(&fr->nbv->grp[eintLocal].nbl_lists,
                             fr, particlePositionArray, f, mdatoms,
    ...
    if (fr->bQMMM)
    {
        update_QMMMrec(cr, fr, particlePositionArray, mdatoms, box, top);
    }

    if ((flags & GMX_FORCE_BONDED) && top->idef.il[F_POSRES].nr > 0)
    {
        posres_wrapper(fplog, flags, bSepDVDL, inputrec, nrnb, top, box, particlePositionArray,
                       enerd, lambda, fr);
    }

    if ((flags & GMX_FORCE_BONDED) && top->idef.il[F_FBPOSRES].nr > 0)
    {
        fbposres_wrapper(inputrec, nrnb, top, box, particlePositionArray, enerd, fr);
    }

    /* Compute the bonded and non-bonded energies and optionally forces */
    do_force_lowlevel(fplog, step, fr, inputrec, &(top->idef),
                      cr, nrnb, wcycle, mdatoms,
                      particlePositionArray, hist, f, bSepLRF ? fr->f_twin : f, enerd, fcd, top, fr->born,
    ...
    if (ed)
    {
        do_flood(cr, inputrec, particlePositionArray, f, ed, box, step, bNS);
    }
    ...
    if (bDoForces)
    {
        ...
        {
            wallcycle_start(wcycle, ewcVSITESPREAD);
            spread_vsite_f(vsite, particlePositionArray, f, fr->fshift, FALSE, NULL, nrnb,
                           &top->idef, fr->ePBC, fr->bMolPBC, graph, box, cr);
            wallcycle_stop(wcycle, ewcVSITESPREAD);
            ...
            {
                wallcycle_start(wcycle, ewcVSITESPREAD);
                spread_vsite_f(vsite, particlePositionArray, fr->f_twin, NULL, FALSE, NULL,
                               nrnb,
                               &top->idef, fr->ePBC, fr->bMolPBC, graph, box, cr);
                wallcycle_stop(wcycle, ewcVSITESPREAD);
            }
        ...
        if (flags & GMX_FORCE_VIRIAL)
        {
            ...
            calc_virial(0, mdatoms->homenr, particlePositionArray, f,
    ...
    if (inputrec->ePull == epullUMBRELLA || inputrec->ePull == epullCONST_F)
    {
         ...
         */
        pull_potential_wrapper(fplog, bSepDVDL, cr, inputrec, box, particlePositionArray,
    ...
    if (bDoForces)
    {
        post_process_forces(cr, step, nrnb, wcycle,
                            top, box, particlePositionArray, f, vir_force, mdatoms, graph, fr, vsite,
                            flags);
    }
                        ...
                        int flags)
{
    ...
    if (fr->ePBC != epbcNONE)
    {
         ...
         * because of pressure coupling or box deformation!
         */
        if (((flags & GMX_FORCE_DYNAMICBOX) && bStateChanged) || inputrec->userreal1 != 0)
        {
            calc_shifts(box, fr->shift_vec);
        }
