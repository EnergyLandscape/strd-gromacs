		...
		int *npme_x, int *npme_y)
{
	...
	// ===================================================================
	//SRDCODE set minimum domdec cell size to 1.2x largest srd cell size
	if(ir->empcType != empcOFF)
	{
		real cellSizeX = box[XX][XX] / ir->mpcInitialCellCount[XX];
		real cellSizeY = box[YY][YY] / ir->mpcInitialCellCount[YY];
		real cellSizeZ = box[ZZ][ZZ] / ir->mpcInitialCellCount[ZZ];

		real maxCellSize = max(cellSizeX, cellSizeY);
		maxCellSize = max(maxCellSize, cellSizeZ);

		real paddingFactor = 1.20; // (ir->epc == epcNO) ? 1.20 : 1.30;
		comm->cellsize_limit = max(comm->cellsize_limit, paddingFactor * maxCellSize);
	}
	//END_SRDCODE
	// ===================================================================
...
// ===================================================================
//SRDCODE functions implementing SRD communication

#ifdef SRD_ROUTING_DEBUG
void get_reference_cell_homes(t_srd* srd, gmx_domdec_t *dd, matrix box)
{
	int d, i;
	ivec ind;
	rvec pos;
	...
	int ci, cj, ck;
	int cc = 0;
	pos[XX] = srd->gridShift[XX];
	for (ci = 0; ci < srd->cellCount[XX]; ci++)
	{
		pos[YY] = srd->gridShift[YY];
		for (cj = 0; cj < srd->cellCount[YY]; cj++)
		{
			pos[ZZ] = srd->gridShift[ZZ];
			for (ck = 0; ck < srd->cellCount[ZZ]; ck++)
			{
				for (d = 0; d < DIM; d++)
				{
					...
					for (i = 0; i < dd->nc[d]; i++)
					{
						if(pos[d] < dd->ma->cell_x[d][i+1])
							break;
					}
					ind[d] = i;
				}
				srd->cellHomeRanks[cc] = dd_index(dd->nc, ind);

				pos[ZZ] += srd->cellSize[ZZ];
				cc++;
			}
			pos[YY] += srd->cellSize[YY];
		}
		pos[XX] += srd->cellSize[XX];
	}
}

void printCell(gmx_int64_t step, t_commrec* cr)
{
	int d;
	gmx_domdec_t* dd;
	dd = cr->dd;

	printf("<Step %"GMX_PRId64"> [Node %d] Domain bounds: (%f - %f) (%f - %f) (%f - %f)\n",
			step, cr->nodeid,
			dd->comm->cell_x0[0], dd->comm->cell_x1[0],
			dd->comm->cell_x0[1], dd->comm->cell_x1[1],
			dd->comm->cell_x0[2], dd->comm->cell_x1[2]);
}

void printAllCells(gmx_int64_t step, gmx_domdec_t *dd)
{
	int d, i;

	for (d = 0; d < dd->npbcdim; d++)
	{
		for(i = 0; i <= dd->nc[d]; i++)
			printf("<Step %"GMX_PRId64"> Domdec boundaries: [%d] [%d] %f\n", step, d, i, dd->ma->cell_x[d][i]);
	}
}
#endif
...
static inline gmx_bool float_float_eq(float a, float b, float tolerance)
{
	return fabs(a - b) < tolerance;
}

static inline gmx_bool rvec_eq(rvec a, rvec b, float tolerance)
{
...
#ifdef SRD_COMM_TEST
void dd_distribute_vec_cr(t_commrec *cr, rvec *v, rvec *lv)
{
	dd_distribute_vec(cr->dd, &cr->dd->comm->cgs_gl, v, lv);
}
#endif

void dd_sendrecv_double(const gmx_domdec_t gmx_unused *dd,
                      int gmx_unused ddimind, int gmx_unused direction,
                      double gmx_unused *buf_s, int gmx_unused n_s,
                      double gmx_unused *buf_r, int gmx_unused n_r)
{
#ifdef GMX_MPI
    int        rank_s, rank_r;
    MPI_Status stat;
    ...
    if (n_s && n_r)
    {
        MPI_Sendrecv(buf_s, n_s*sizeof(double), MPI_BYTE, rank_s, 0,
                     buf_r, n_r*sizeof(double), MPI_BYTE, rank_r, 0,
                     dd->mpi_comm_all, &stat);
    }
    else if (n_s)
    {
        MPI_Send(    buf_s, n_s*sizeof(double), MPI_BYTE, rank_s, 0,
                     dd->mpi_comm_all);
    }
    else if (n_r)
    {
        MPI_Recv(    buf_r, n_r*sizeof(double), MPI_BYTE, rank_r, 0,
                     dd->mpi_comm_all, &stat);
    }
...
/**
 * Special sendrecv call for dmatrix.
 */
void dd_sendrecv_dmat(const gmx_domdec_t gmx_unused *dd,
		int gmx_unused ddimind, int gmx_unused direction,
		dmatrix gmx_unused *buf_s, int gmx_unused n_s,
		dmatrix gmx_unused *buf_r, int gmx_unused n_r)
{
#ifdef GMX_MPI
	int        rank_s, rank_r;
	MPI_Status stat;
	...
	if (n_s && n_r)
	{
		MPI_Sendrecv(buf_s[0], n_s*sizeof(dmatrix), MPI_BYTE, rank_s, 0,
				buf_r[0], n_r*sizeof(dmatrix), MPI_BYTE, rank_r, 0,
				dd->mpi_comm_all, &stat);
	}
	else if (n_s)
	{
		MPI_Send(    buf_s[0], n_s*sizeof(dmatrix), MPI_BYTE, rank_s, 0,
				dd->mpi_comm_all);
	}
	else if (n_r)
	{
		MPI_Recv(    buf_r[0], n_r*sizeof(dmatrix), MPI_BYTE, rank_r, 0,
				dd->mpi_comm_all, &stat);
	}
...
/**
 * Special sendrecv call for MPC particle metadata.
 */
void dd_sendrecv_srd_mdata(const gmx_domdec_t gmx_unused *dd,
		int gmx_unused ddimind, int gmx_unused direction,
		srd_mdata gmx_unused *buf_s, int gmx_unused n_s,
		srd_mdata gmx_unused *buf_r, int gmx_unused n_r)
{
#ifdef GMX_MPI
	int        rank_s, rank_r;
	MPI_Status stat;
	...
	if (n_s && n_r)
	{
		MPI_Sendrecv(buf_s[0], n_s*sizeof(srd_mdata), MPI_BYTE, rank_s, 0,
				buf_r[0], n_r*sizeof(srd_mdata), MPI_BYTE, rank_r, 0,
				dd->mpi_comm_all, &stat);
	}
	else if (n_s)
	{
		MPI_Send(    buf_s[0], n_s*sizeof(srd_mdata), MPI_BYTE, rank_s, 0,
				dd->mpi_comm_all);
	}
	else if (n_r)
	{
		MPI_Recv(    buf_r[0], n_r*sizeof(srd_mdata), MPI_BYTE, rank_r, 0,
				dd->mpi_comm_all, &stat);
	}
...
/**
 * Creates a local routing map that gives the communication pulse ID
 * required to send data to a particular destination node.
 *
 * Values in the map are pulseIDs or -1 if the destination node
 * cannot be reached.
 */
static inline void create_routing_map(t_srd* srd, t_commrec* cr)
{
	int d, dim, ci, i, j, k;
	gmx_domdec_t* dd;
	dd = cr->dd;

	// collect node IDs of neighboring domain decomposition nodes

	for(i = 0; i < 3; i++)
		for(j = 0; j < 3; j++)
			for(k = 0; k < 3; k++)
				srd->neighbors[i][j][k] = SRD_NO_NEIGHBOR;
	srd->neighbors[1][1][1] = cr->nodeid;

	d = dd->ndim-1;
	if(dd->nc[ZZ] > 1)
	{
		dd_sendrecv_int(dd, d, dddirForward,
				&srd->neighbors[1][1][1], 1,
				&srd->neighbors[1][1][0], 1);

		if(dd->nc[ZZ] > 2)
		{
			dd_sendrecv_int(dd, d, dddirBackward,
					&srd->neighbors[1][1][1], 1,
					&srd->neighbors[1][1][2], 1);
		}
		d--;
	}
	...
	{
		dd_sendrecv_int(dd, d, dddirForward,
				&srd->neighbors[1][1][0], 3,
				&srd->neighbors[1][0][0], 3);

		if(dd->nc[YY] > 2)
		{
			dd_sendrecv_int(dd, d, dddirBackward,
					&srd->neighbors[1][1][0], 3,
					&srd->neighbors[1][2][0], 3);
		}
		d--;
	}
	...
	{
		dd_sendrecv_int(dd, d, dddirForward,
				&srd->neighbors[1][0][0], 9,
				&srd->neighbors[0][0][0], 9);

		if(dd->nc[XX] > 2)
		{
			dd_sendrecv_int(dd, d, dddirBackward,
					&srd->neighbors[1][0][0], 9,
					&srd->neighbors[2][0][0], 9);
		}
		d--;
	}

	snew(srd->nodeRouting, cr->nnodes);

	// unreachable nodes are indiciated by -1 in the routing map
	for(ci = 0; ci < cr->nnodes; ci++)
		srd->nodeRouting[ci] = -1;

	// assign pulse IDs to nodes in neighborIDs

	for(j = 0; j < 3; j++)
		for(k = 0; k < 3; k++)
		{
			if(srd->neighbors[0][j][k] != SRD_NO_NEIGHBOR)
				srd->nodeRouting[srd->neighbors[0][j][k]] = 0;

			if(srd->neighbors[2][j][k] != SRD_NO_NEIGHBOR)
				srd->nodeRouting[srd->neighbors[2][j][k]] = 1;
		}

	for(k = 0; k < 3; k++)
	{
		if(srd->neighbors[1][0][k] != SRD_NO_NEIGHBOR)
			srd->nodeRouting[srd->neighbors[1][0][k]] = 2;

		if(srd->neighbors[1][2][k] != SRD_NO_NEIGHBOR)
			srd->nodeRouting[srd->neighbors[1][2][k]] = 3;
	}

	if(srd->neighbors[1][1][0] != SRD_NO_NEIGHBOR)
		srd->nodeRouting[srd->neighbors[1][1][0]] = 4;

	if(srd->neighbors[1][1][2] != SRD_NO_NEIGHBOR)
		srd->nodeRouting[srd->neighbors[1][1][2]] = 5;

#ifdef SRD_LOCAL_LOGS
	fprintf(srd->localLogFP, "cell bounds: (%4.1f - %4.1f) (%4.1f - %4.1f) (%4.1f - %4.1f)\n",
			cr->dd->comm->cell_x0[XX], cr->dd->comm->cell_x1[XX],
			cr->dd->comm->cell_x0[YY], cr->dd->comm->cell_x1[YY],
			cr->dd->comm->cell_x0[ZZ], cr->dd->comm->cell_x1[ZZ]);

	fprintf(srd->localLogFP, "neighbors:\n");
	for(k = 0; k < 3; k++)
	{
		for(j = 0; j < 3; j++)
		{
			for(i = 0; i < 3; i++)
			{
				if(srd->neighbors[i][j][k] != SRD_NO_NEIGHBOR)
					fprintf(srd->localLogFP, "%3d ", srd->neighbors[i][j][k]);
				else
					fprintf(srd->localLogFP, "  X ");
			}
...
void get_dd_bounds(t_commrec* cr, rvec x0, rvec x1)
{
	copy_rvec(cr->dd->comm->cell_x0, x0);
	copy_rvec(cr->dd->comm->cell_x1, x1);
}

/*
 * Communicates with neighbors to sets the values of srd->boundaries,
 * the DD boundaries of each node (no triclinic boxes).
 */
void get_neighboring_boundaries(t_srd* srd, t_commrec* cr)
{
	int d, dim, ci, i, j, k;
	gmx_domdec_t* dd;
	dd = cr->dd;
	...
	for(k = 0; k < 3; k++)
	{
		for(dim = 0; dim < DIM; dim++)
		{
			srd->boundaries[i][j][k][0][dim] = NAN;
			srd->boundaries[i][j][k][1][dim] = NAN;
		}
	}

	copy_rvec(dd->comm->cell_x0, srd->boundaries[1][1][1][0]);
	copy_rvec(dd->comm->cell_x1, srd->boundaries[1][1][1][1]);

	d = dd->ndim-1;
	if(dd->nc[ZZ] > 1)
	{
		dd_sendrecv_rvec(dd, d, dddirForward,
				&srd->boundaries[1][1][1][0], 2,
				&srd->boundaries[1][1][0][0], 2);

		if(dd->nc[ZZ] > 2)
		{
			dd_sendrecv_rvec(dd, d, dddirBackward,
					&srd->boundaries[1][1][1][0], 2,
					&srd->boundaries[1][1][2][0], 2);
		}
		d--;
	}
	...
	{
		dd_sendrecv_rvec(dd, d, dddirForward,
				&srd->boundaries[1][1][0][0], 6,
				&srd->boundaries[1][0][0][0], 6);

		if(dd->nc[YY] > 2)
		{
			dd_sendrecv_rvec(dd, d, dddirBackward,
					&srd->boundaries[1][1][0][0], 6,
					&srd->boundaries[1][2][0][0], 6);
		}
		d--;
	}
	...
	{
		dd_sendrecv_rvec(dd, d, dddirForward,
				&srd->boundaries[1][0][0][0], 18,
				&srd->boundaries[0][0][0][0], 18);

		if(dd->nc[XX] > 2)
		{
			dd_sendrecv_rvec(dd, d, dddirBackward,
					&srd->boundaries[1][0][0][0], 18,
					&srd->boundaries[2][0][0][0], 18);
		}
		d--;
	}
	...
	for(i = 0; i < 3; i++)
		for(j = 0; j < 3; j++)
			for(k = 0; k < 3; k++)
			{
				...
				fprintf(srd->localLogFP, "[%d %d %d] (%8.4f to %8.4f) (%8.4f to %8.4f) (%8.4f to %8.4f)\n",
						i, j, k,
						srd->boundaries[i][j][k][0][XX], srd->boundaries[i][j][k][1][XX],
						srd->boundaries[i][j][k][0][YY], srd->boundaries[i][j][k][1][YY],
						srd->boundaries[i][j][k][0][ZZ], srd->boundaries[i][j][k][1][ZZ]);
			}
#endif
}

void dd_srd_init(t_srd* srd, t_commrec* cr)
{
	create_routing_map(srd, cr);
	get_neighboring_boundaries(srd, cr);
}

void dd_srd_update(t_srd* srd, t_commrec* cr)
{
	if(cr->dd->comm->bDynLoadBal)
	{
		get_neighboring_boundaries(srd, cr);
	}
}

// dump all relevant information and call gmx_fatal
static void ERROR_NO_PULSE_BEFORE(t_srd* srd, t_commrec* cr, srd_mdata mdata, int pulse)
{
	int i, j, k;
	for(i = 0; i < 3; i++)
	for(j = 0; j < 3; j++)
	for(k = 0; k < 3; k++)
	{
		...
		printf("<Node %d> Neighbor: [node %d] [%d %d %d] (%d to %d) (%d to %d) (%d to %d) -- (%f to %f) (%f to %f) (%f to %f)\n",
			cr->nodeid, srd->neighbors[i][j][k], i, j, k,
			...
			srd->boundaries[i][j][k][0][XX] - srd->gridShift[XX], srd->boundaries[i][j][k][1][XX] - srd->gridShift[XX],
			srd->boundaries[i][j][k][0][YY] - srd->gridShift[YY], srd->boundaries[i][j][k][1][YY] - srd->gridShift[YY],
			srd->boundaries[i][j][k][0][ZZ] - srd->gridShift[ZZ], srd->boundaries[i][j][k][1][ZZ] - srd->gridShift[ZZ]);
	}
	printf("Cell count = %d %d %d\n", srd->cellCount[XX], srd->cellCount[YY], srd->cellCount[ZZ]);
	...
	int cellID = mdata[MD_CELL];
	int ci, cj, ck;
	ci = cj = ck = -1;

	if(cellID >= 0 && cellID < srd->numCells)
		getCellIndicies(srd, cellID, &ci, &cj, &ck);

	gmx_fatal(FARGS,
			"<Step %"GMX_PRId64"> [Node %d] Unable to route SRD particle before collision!\n"
			"Atom %d from node %d is trying to get to cell %d (%d %d %d) via pulse %d\n",
				srd->step, cr->nodeid, mdata[MD_GNDX], mdata[MD_NODE],
				cellID, ci, cj, ck, pulse);
}

// dump all relevant information and call gmx_fatal
static void ERROR_NO_PULSE_AFTER(t_srd* srd, t_commrec* cr, srd_mdata mdata, int pulse)
{
	int i, j, k;
	printf("<Step %"GMX_PRId64"> neighboring cells:\n", srd->step);
	for(i = 0; i < 3; i++)
	for(j = 0; j < 3; j++)
	for(k = 0; k < 3; k++)
	{
		...
		printf("[node %d] [%d %d %d] (%d to %d) (%d to %d) (%d to %d) -- (%f to %f) (%f to %f) (%f to %f)\n",
	...
	gmx_fatal(FARGS,
			"<Step %"GMX_PRId64"> [Node %d] Unable to route SRD particle after collision!\n"
			"Atom %d is trying to return to node %d via pulse %d\n",
				srd->step, cr->nodeid, mdata[MD_GNDX], mdata[MD_NODE], pulse);
}

/**
 * Copies collision particle data for the current communication pulse from the dispatch buffer
 * to the send buffer. Called during communication before the collision step.
 */
static inline int copy_dispatch_to_send_before(t_srd* srd, int ndispatch, int pulse, t_commrec* cr)
{
	// allocate space for send buffer if necessary
	realloc_srd_comm_buffer(srd, srd->sendBuffer, ndispatch);

	int ai;
	int nsend = 0;
	for(ai = 0; ai < ndispatch; ai++)
	{
		if(srd->dispatched[ai])
			continue;

		int cellID = srd->dispatchBuffer->M[ai][MD_CELL];
		int pulseID = srd->cellRouting[cellID];

		/**
		 * This should not happen. If it does, we have local SRD particles with no way to
		 * reach their assigned collision cell.
		 */
		if(pulseID < 0)
			ERROR_NO_PULSE_BEFORE(srd, cr, srd->dispatchBuffer->M[ai], pulse);

		#ifdef SRD_ROUTING_DEBUG
		int nodeID = srd->cellHomeRanks[cellID];
		int pulseID_check = srd->nodeRouting[nodeID];

		if (pulseID != pulseID_check)
		{
			ivec xyzHere;
			ivec xyzThere;
			ddindex2xyz(cr->dd->nc, cr->nodeid, xyzHere);
			ddindex2xyz(cr->dd->nc, nodeID, xyzThere);
			printf("<Step %"GMX_PRId64"> [Node %d] ERROR: (Phase 1) pulseIDs diverge %d vs %d (expected).\n", srd->step, cr->nodeid, pulseID, pulseID_check);
			printf("[Node %d] (%d %d %d) ERROR: (Phase 1) Incorrect cell %d is on node %d (%d %d %d).\n",
					cr->nodeid, xyzHere[XX], xyzHere[YY], xyzHere[ZZ],
					cellID, nodeID, xyzThere[XX], xyzThere[YY], xyzThere[ZZ]);
		}
		...
		if(pulseID < 0)
		{
			...
			ddindex2xyz(cr->dd->nc, cr->nodeid, xyzHere);
			ddindex2xyz(cr->dd->nc, nodeID, xyzThere);
			printf("<Step %"GMX_PRId64"> [Node %d] (%d %d %d) ERROR: (Phase 1) Impossible cell %d is on node %d (%d %d %d).\n",
					srd->step, cr->nodeid, xyzHere[XX], xyzHere[YY], xyzHere[ZZ],
					cellID, nodeID, xyzThere[XX], xyzThere[YY], xyzThere[ZZ]);
		}
		#endif
		...
		if(pulse == pulseID)
		{
			if(srd->bCommunicateX)
				copy_rvec(srd->dispatchBuffer->X[ai], srd->sendBuffer->X[nsend]);
			copy_rvec(srd->dispatchBuffer->V[ai], srd->sendBuffer->V[nsend]);
			copy_srd_mdata(srd->dispatchBuffer->M[ai], srd->sendBuffer->M[nsend]);
			srd->dispatched[ai] = TRUE;
			nsend++;
		}
	}
	return nsend;
...
/**
 * Copies collision particle data for the current communication pulse from the dispatch buffer
 * to the send buffer. Called during communication after the collision step.
 */
static inline int copy_dispatch_to_send_after(t_srd* srd, int ndispatch, int pulse, t_commrec* cr)
{
	// allocate space for send buffer if necessary
	realloc_srd_comm_buffer(srd, srd->sendBuffer, ndispatch);
	...
	for(ai = 0; ai < ndispatch; ai++)
	{
		...
		int nodeID = srd->dispatchBuffer->M[ai][MD_NODE];
		int pulseID = srd->nodeRouting[nodeID];

		/**
		 * This should not happen. If it does, we have adopted SRD particles with no way to
		 * get them back to their home node.
		 */
		if(pulseID < 0)
			ERROR_NO_PULSE_AFTER(srd, cr, srd->dispatchBuffer->M[ai], pulse);
...
void dd_before_srd_collisions(t_srd* srd, t_state* state, t_commrec* cr)
{
	int ai;
	int nsendBackward, nsendForward;
	int nrecvBackward, nrecvForward;
	...
	// ======================================================================
	// copy home SRD atoms to dispatch buffer
	// ======================================================================

	int ndispatch = 0;

	// allocate space for dispatch buffer if necessary
	if(realloc_srd_comm_buffer(srd, srd->dispatchBuffer, srd->homenr))
		srenew(srd->dispatched, srd->dispatchBuffer->capacity);
	...
	for (ai = 0; ai < srd->homenr; ai++)
	{
		...
		if(srd->bCommunicateX)
			copy_rvec(srd->relXBuf->buf[ai], srd->dispatchBuffer->X[ndispatch]);
		copy_rvec(state->v[ai], srd->dispatchBuffer->V[ndispatch]);
		srd->dispatchBuffer->M[ndispatch][MD_NODE] = cr->nodeid;
		srd->dispatchBuffer->M[ndispatch][MD_GNDX] = dd->gatindex[ai];
		srd->dispatchBuffer->M[ndispatch][MD_LNDX] = ai;
		srd->dispatchBuffer->M[ndispatch][MD_CELL] = cellID;
		srd->dispatched[ndispatch] = FALSE;
		ndispatch++;

#ifdef SRD_COMM_DEBUG
		srd->atomState[ai] = STATE_SENT;
#endif
	}

#ifdef SRD_COMM_DEBUG
	srd->totalSent = ndispatch;
	fprintf(srd->localLogFP, "\nStep %"GMX_PRId64" -- Before SRD Collisions\n", srd->step);
	fprintf(srd->localLogFP, "Shift = %f %f %f\n", srd->gridShift[XX], srd->gridShift[YY], srd->gridShift[ZZ]);
	fprintf(srd->localLogFP, "Copied %d / %d local atoms to the dispatch buffer.\n", ndispatch, srd->homenr);
#endif
	...
	for (dim = 0; dim < DIM; dim++)
	{
		...
		// ======================================================================
		// backward communication pulse (only if >1 dd zones along this dim)
		// ======================================================================
		...
		nsendBackward = copy_dispatch_to_send_before(srd, ndispatch, pulse++, cr);
...
#ifdef SRD_COMM_DEBUG
		fprintf(srd->localLogFP, "(Pulse %d) Sending: %3d -> %2d Receiving: %2d -> %3d\n",
				pulse,
				nsendBackward, dd->neighbor[d][1],
				dd->neighbor[d][0], nrecvBackward);
#endif

		// allocate space for recv buffer if necessary
		realloc_srd_comm_buffer(srd, srd->recvBuffer, nrecvBackward);

		if(srd->bCommunicateX)
		{
			dd_sendrecv_rvec(dd, d, dddirBackward,
					srd->sendBuffer->X, nsendBackward,
					srd->recvBuffer->X, nrecvBackward);
		}

		dd_sendrecv_rvec(dd, d, dddirBackward,
				srd->sendBuffer->V, nsendBackward,
				srd->recvBuffer->V, nrecvBackward);

		dd_sendrecv_srd_mdata(dd, d, dddirBackward,
				srd->sendBuffer->M, nsendBackward,
				srd->recvBuffer->M, nrecvBackward);

		// ======================================================================
		// forward communication pulse (only if >2 dd zones along this dim)
		// ======================================================================

		if(dd->nc[dim] > 2)
		{
			nsendForward = copy_dispatch_to_send_before(srd, ndispatch, pulse++, cr);
			...
			fprintf(srd->localLogFP, "(Pulse %d) Sending: %3d -> %2d Receiving: %2d -> %3d\n",
					pulse,
					nsendForward, dd->neighbor[d][0],
					dd->neighbor[d][1], nrecvForward);
#endif

			// allocate space for recv buffer if necessary
			realloc_srd_comm_buffer(srd, srd->recvBuffer, nrecvBackward + nrecvForward);

			if(srd->bCommunicateX)
			{
				dd_sendrecv_rvec(dd, d, dddirForward,
						srd->sendBuffer->X, nsendForward,
						srd->recvBuffer->X + nrecvBackward, nrecvForward);
			}

			dd_sendrecv_rvec(dd, d, dddirForward,
					srd->sendBuffer->V, nsendForward,
					srd->recvBuffer->V + nrecvBackward, nrecvForward);

			dd_sendrecv_srd_mdata(dd, d, dddirForward,
					srd->sendBuffer->M, nsendForward,
					srd->recvBuffer->M + nrecvBackward, nrecvForward);
		}
		else
		{
			pulse++;
		...
		// ======================================================================
		// sort received particles (adopt or dispatch)
		// ======================================================================

		int adoptedBackward = 0;
		int adoptedForward = 0;
		int numForwarding = 0;
		for (ai = 0; ai < (nrecvBackward + nrecvForward); ai++)
		{
			int cellID = srd->recvBuffer->M[ai][MD_CELL];

			if(srd->isLocalCell[cellID])
			{
				// allocate space for adoption buffer if necessary
				realloc_srd_comm_buffer(srd, srd->adoptionBuffer, srd->adoptedCount + 1);

				// copy adopted SRD particles to local buffer
				if(srd->bCommunicateX)
					copy_rvec(srd->recvBuffer->X[ai], srd->adoptionBuffer->X[srd->adoptedCount]);
				copy_rvec(srd->recvBuffer->V[ai], srd->adoptionBuffer->V[srd->adoptedCount]);
				copy_srd_mdata(srd->recvBuffer->M[ai], srd->adoptionBuffer->M[srd->adoptedCount]);
				srd->adoptedCount++;
				if(ai < nrecvBackward)
					adoptedBackward++;
				else
			...
			{
				// allocate space for dispatch buffer if necessary
				if(realloc_srd_comm_buffer(srd, srd->dispatchBuffer, ndispatch + 1))
					srenew(srd->dispatched, srd->dispatchBuffer->capacity);

				// unless this is the last pulse, copy remaining to send buffer for next pulse
				if(srd->bCommunicateX)
					copy_rvec(srd->recvBuffer->X[ai], srd->dispatchBuffer->X[ndispatch]);
				copy_rvec(srd->recvBuffer->V[ai], srd->dispatchBuffer->V[ndispatch]);
				copy_srd_mdata(srd->recvBuffer->M[ai], srd->dispatchBuffer->M[ndispatch]);
				srd->dispatched[ndispatch] = FALSE;
				ndispatch++;
				numForwarding++;
			}
...
#ifdef SRD_COMM_DEBUG
		fprintf(srd->localLogFP, "Adopted: %3d (%3d + %3d), Forwarding: %3d\n",
				adoptedBackward + adoptedForward, adoptedBackward, adoptedForward, numForwarding);
#endif
	}
}

void dd_after_srd_collisions(t_srd* srd, t_state* state, t_commrec* cr)
{
	int ai;
	int nsendBackward, nsendForward;
	int nrecvBackward, nrecvForward;
	...
	// ======================================================================
	// copy adopted SRD atoms to dispatch buffer
	// ======================================================================

	int ndispatch = srd->adoptedCount;

	// allocate space for dispatch buffer if necessary
	if(realloc_srd_comm_buffer(srd, srd->dispatchBuffer, ndispatch))
		srenew(srd->dispatched, srd->dispatchBuffer->capacity);

	// faster than using a loop
	memcpy(srd->dispatchBuffer->V, srd->adoptionBuffer->V, ndispatch*sizeof(rvec));
	memcpy(srd->dispatchBuffer->M, srd->adoptionBuffer->M, ndispatch*sizeof(srd_mdata));
	memset(srd->dispatched, FALSE, ndispatch*sizeof(gmx_bool));

#ifdef SRD_COMM_DEBUG
	for (ai = 0; ai < srd->adoptedCount; ai++)
		dest[srd->adoptionBuffer->M[ai][MD_NODE]]++;
#endif
	...
	{
		if(dest[i] > 0)
			fprintf(srd->localLogFP, "[Node %d] %4d SRD particles for node %d (pulse %d)\n",
					cr->nodeid, dest[i], i, srd->nodeRouting[i]);
	}

	fprintf(srd->localLogFP, "\nStep %"GMX_PRId64" -- After SRD Collisions\n", srd->step);
	srd->totalReturned = 0;
#endif
	...
	for (dim = 0; dim < DIM; dim++)
	{
		...
		nsendBackward = copy_dispatch_to_send_after(srd, ndispatch, pulse++, cr);
		...
		if(dd->nc[dim] > 2)
		{
			nsendForward = copy_dispatch_to_send_after(srd, ndispatch, pulse++, cr);
		...
		// ======================================================================
		// keep returning particles, copy others to next pulse
		// ======================================================================

		int numReclaimed = 0;
		int numForwarding = 0;
		for (ai = 0; ai < (nrecvForward + nrecvBackward); ai++)
		{
			if(srd->recvBuffer->M[ai][MD_NODE] == cr->nodeid)
			{
				int localIndex = srd->recvBuffer->M[ai][MD_LNDX];
				copy_rvec(srd->recvBuffer->V[ai], state->v[localIndex]);
				numReclaimed++;

#ifdef SRD_COMM_DEBUG
				int globalIndex = dd->gatindex[localIndex];

				if (srd->atomState[localIndex] != STATE_SENT)
					printf("[Node %d][Atom %d] ERROR: Received SRD particle with invalid state: %d \n",
							cr->nodeid, globalIndex, srd->atomState[localIndex]);
				else
					srd->atomState[localIndex] = STATE_RETURNED;

				if (!rvec_eq(srd->testVp[globalIndex], srd->recvBuffer->V[ai], 1e-6))
				{
					fprintf(srd->localLogFP, "[Atom %d] ERROR: Inconsistent SRD velocity: (%f %f %f) vs (%f %f %f)\n",
							globalIndex,
							srd->testVp[globalIndex][XX], srd->testVp[globalIndex][YY], srd->testVp[globalIndex][ZZ],
							srd->recvBuffer->V[ai][XX], srd->recvBuffer->V[ai][YY], srd->recvBuffer->V[ai][ZZ]);
					printf("[Node %d][Atom %d] ERROR: Inconsistent SRD velocity: (%f %f %f) vs (%f %f %f)\n",
			...
			else
			{
				...
				// copy to dispatch buffer for next pulse
				copy_rvec(srd->recvBuffer->V[ai], srd->dispatchBuffer->V[ndispatch]);
				copy_srd_mdata(srd->recvBuffer->M[ai], srd->dispatchBuffer->M[ndispatch]);
				srd->dispatched[ndispatch] = FALSE;
				ndispatch++;
...
#ifdef SRD_COMM_DEBUG
		srd->totalReturned += numReclaimed;;
		fprintf(srd->localLogFP, "Reclaimed: %d, Forwarding: %d (ndispatch = %d)\n", numReclaimed, numForwarding, ndispatch);
#endif
	}

#ifdef SRD_COMM_DEBUG
	fprintf(srd->localLogFP, "Final count: %d / %d\n", srd->totalReturned, srd->totalSent);
	if(srd->totalReturned != srd->totalSent)
		printf("[%d] ERROR: Not all SRD particles were updated! %d / %d\n", cr->nodeid, srd->totalReturned, srd->totalSent);
#endif
}
 ...
 *
 */
void dd_srd_find_local_collision_cells(t_srd* srd, t_commrec* cr)
{
	gmx_domdec_t* dd;
	dd = cr->dd;
	int d, cd, sd, i;
	...
	// find cell ranges for local and shared cells
	for(d = 0; d < DIM; d++)
	{
		if(cr->dd->nc[d] < 2)
		{
			srd->sharedCellCountB[d] = 0;
			srd->sharedCellCountF[d] = 0;
			srd->localCellCount[d] = srd->cellCount[d];

			for (cd = 0; cd < srd->cellCount[d]; cd++)
				srd->localCells[d][cd] = cd;

			continue;
		}

		real size = srd->cellSize[d];
		real x0 = dd->comm->cell_x0[d];
		real x1 = dd->comm->cell_x1[d];
		real shift = srd->gridShift[d];
		real padding = size / 2;

		int lmin = floor((x0 - shift - padding) / size); // inclusive
		int lmax = floor((x0 - shift + padding) / size); // inclusive

		int rmin = floor((x1 - shift - padding) / size); // inclusive
		int rmax = floor((x1 - shift + padding) / size); // inclusive

		cmin[d] = lmin;
		cmax[d] = rmax;

		i = 0;
		for (cd = lmin; cd <= lmax; cd++)
		{
			sd = cd;
			if (sd < 0)
				sd += srd->cellCount[d];
			else if (sd >= srd->cellCount[d])
				sd -= srd->cellCount[d];

			srd->sharedCellsB[d][i++] = sd;
		}
		srd->sharedCellCountB[d] = i;

		i = 0;
		for (cd = rmin; cd <= rmax; cd++)
		{
			sd = cd;
			if (sd < 0)
				sd += srd->cellCount[d];
			...
			srd->sharedCellsF[d][i++] = sd;
		}
		srd->sharedCellCountF[d] = i;

		i = 0;
		for (cd = lmin; cd <= rmax; cd++)
		{
			sd = cd;
			if (sd < 0)
				sd += srd->cellCount[d];
			...
			srd->localCells[d][i++] = sd;
		}
		srd->localCellCount[d] = i;
	}
	...
	for (rj = 0; rj < srd->localCellCount[YY]; rj++)
	{
		...
		for (rk = 0; rk < srd->localCellCount[ZZ]; rk++)
		{
			...
			srd->isLocalCell[ci] = TRUE;
		}
	}
}

static gmx_inline int dd_srd_get_cell_count(t_srd* srd, int d, int sharedCellCount[3], int* sharedCells[3])
{
	int ri, rj, rk;
	int ncells = 0;
	...
	{
	case XX:
		for (ri = 0; ri < sharedCellCount[XX]; ri++)
		for (rj = 0; rj < srd->localCellCount[YY]; rj++)
		{
			int cxy = (srd->cellCount[ZZ] * ((srd->cellCount[YY] * sharedCells[XX][ri]) + srd->localCells[YY][rj]));
			for (rk = 0; rk < srd->localCellCount[ZZ]; rk++)
			{
				int ci = cxy + srd->localCells[ZZ][rk];
				if(srd->cellOccupancy[ci] > 0)
					ncells++;
			}
		}
		break;
	case YY:
		for (ri = 0; ri < srd->localCellCount[XX]; ri++)
		for (rj = 0; rj < sharedCellCount[YY]; rj++)
		{
			int cxy = (srd->cellCount[ZZ] * ((srd->cellCount[YY] * srd->localCells[XX][ri]) + sharedCells[YY][rj]));
			for (rk = 0; rk < srd->localCellCount[ZZ]; rk++)
			{
				int ci = cxy + srd->localCells[ZZ][rk];
				if(srd->cellOccupancy[ci] > 0)
	...
	case ZZ:
		...
		{
			int cxy = (srd->cellCount[ZZ] * ((srd->cellCount[YY] * srd->localCells[XX][ri]) + srd->localCells[YY][rj]));
			for (rk = 0; rk < sharedCellCount[ZZ]; rk++)
			{
				int ci = cxy + sharedCells[ZZ][rk];
				if(srd->cellOccupancy[ci] > 0)
					ncells++;
			}
		}
		break;
	default:
		gmx_fatal(FARGS, "Invalid cell dimension %d\n", d);
	}
	return ncells;
}

static gmx_inline void load_rvec(t_srd* srd, const rvec a, int *nr)
{
	srd->sbuf_real->buf[(*nr)++] = a[XX];
	srd->sbuf_real->buf[(*nr)++] = a[YY];
	srd->sbuf_real->buf[(*nr)++] = a[ZZ];
}

static gmx_inline void accumulate_rvec(t_srd* srd, rvec a, int *nr)
{
	a[XX] += srd->rbuf_real->buf[(*nr)++];
	a[YY] += srd->rbuf_real->buf[(*nr)++];
	a[ZZ] += srd->rbuf_real->buf[(*nr)++];
}

static gmx_inline void load_dmat(t_srd* srd, dmatrix a, int *nd)
{
	srd->sbuf_double->buf[(*nd)++] = a[XX][XX];
	srd->sbuf_double->buf[(*nd)++] = a[XX][YY];
	srd->sbuf_double->buf[(*nd)++] = a[XX][ZZ];
	srd->sbuf_double->buf[(*nd)++] = a[YY][XX];
	srd->sbuf_double->buf[(*nd)++] = a[YY][YY];
	srd->sbuf_double->buf[(*nd)++] = a[YY][ZZ];
	srd->sbuf_double->buf[(*nd)++] = a[ZZ][XX];
	srd->sbuf_double->buf[(*nd)++] = a[ZZ][YY];
	srd->sbuf_double->buf[(*nd)++] = a[ZZ][ZZ];
}

static gmx_inline void accumulate_dmat(t_srd* srd, dmatrix a, int *nd)
{
	a[XX][XX] += srd->rbuf_double->buf[(*nd)++];
	a[XX][YY] += srd->rbuf_double->buf[(*nd)++];
	a[XX][ZZ] += srd->rbuf_double->buf[(*nd)++];
	a[YY][XX] += srd->rbuf_double->buf[(*nd)++];
	a[YY][YY] += srd->rbuf_double->buf[(*nd)++];
	a[YY][ZZ] += srd->rbuf_double->buf[(*nd)++];
	a[ZZ][XX] += srd->rbuf_double->buf[(*nd)++];
	a[ZZ][YY] += srd->rbuf_double->buf[(*nd)++];
	a[ZZ][ZZ] += srd->rbuf_double->buf[(*nd)++];
}

static gmx_inline void dd_srd_load_ci(t_srd* srd, int *ni, int *nr, int *nd, int ci, int flags)
{
	srd->sbuf_int->buf[(*ni)++] = ci;

	if (flags & SRD_COMM_FLAGS_SUM_N)
		srd->sbuf_int->buf[(*ni)++] = srd->cellOccupancy[ci];

	if (flags & SRD_COMM_FLAGS_SUM_X)
		load_rvec(srd, srd->cellMeanX[ci], nr);

	if (flags & SRD_COMM_FLAGS_SUM_V)
		load_rvec(srd, srd->cellMeanV[ci], nr);

	if (flags & SRD_COMM_FLAGS_SUM_V2)
		srd->sbuf_real->buf[(*nr)++] = srd->cellVRel2[ci];

	if (flags & SRD_COMM_FLAGS_SUM_RAND)
		load_rvec(srd, srd->cellRandomSum[ci], nr);

	if (flags & SRD_COMM_FLAGS_SUM_DL)
		load_rvec(srd, srd->cellDeltaL[ci], nr);

	if (flags & SRD_COMM_FLAGS_SUM_MIT)
		load_dmat(srd, srd->cellInertiaTensor[ci], nd);

	if (flags & SRD_COMM_FLAGS_SUM_LI)
		load_rvec(srd, srd->initialAngularMomentum[ci], nr);

	if (flags & SRD_COMM_FLAGS_SUM_LF)
		load_rvec(srd, srd->finalAngularMomentum[ci], nr);
}

static gmx_inline void dd_srd_accumulate(t_srd* srd, int nrecv, int flags)
{
	int ci, i;
	...
	for(i = 0; i < nrecv; i++)
	{
		ci = srd->rbuf_int->buf[ni++];

		if (flags & SRD_COMM_FLAGS_SUM_N)
			srd->cellOccupancy[ci] += srd->rbuf_int->buf[ni++];

		if (flags & SRD_COMM_FLAGS_SUM_X)
			accumulate_rvec(srd, srd->cellMeanX[ci], &nf);

		if (flags & SRD_COMM_FLAGS_SUM_V)
			accumulate_rvec(srd, srd->cellMeanV[ci], &nf);

		if (flags & SRD_COMM_FLAGS_SUM_V2)
			srd->cellVRel2[ci] += srd->rbuf_real->buf[nf++];

		if (flags & SRD_COMM_FLAGS_SUM_RAND)
			accumulate_rvec(srd, srd->cellRandomSum[ci], &nf);

		if (flags & SRD_COMM_FLAGS_SUM_DL)
			accumulate_rvec(srd, srd->cellDeltaL[ci], &nf);

		if (flags & SRD_COMM_FLAGS_SUM_MIT)
			accumulate_dmat(srd, srd->cellInertiaTensor[ci], &nd);

		if (flags & SRD_COMM_FLAGS_SUM_LI)
			accumulate_rvec(srd, srd->initialAngularMomentum[ci], &nf);

		if (flags & SRD_COMM_FLAGS_SUM_LF)
			accumulate_rvec(srd, srd->finalAngularMomentum[ci], &nf);
	}
}

static gmx_inline int dd_srd_load(t_srd* srd, int d, int sharedCellCount[3], int* sharedCells[3], int flags)
{
	int ri, rj, rk;

	int ncells = dd_srd_get_cell_count(srd, d, sharedCellCount, sharedCells);
	int numInt = 1;
	int numReal = 0;
	int numDouble = 0;
	...
	realloc_srd_int_buffer(srd, srd->sbuf_int, ncells * numInt);
	realloc_srd_real_buffer(srd, srd->sbuf_real, ncells * numReal);
	realloc_srd_double_buffer(srd, srd->sbuf_double, ncells * numDouble);
	...
	case XX:
		...
		for (rj = 0; rj < srd->localCellCount[YY]; rj++)
		{
			...
			for (rk = 0; rk < srd->localCellCount[ZZ]; rk++)
			{
				int ci = cxy + srd->localCells[ZZ][rk];
				if(srd->cellOccupancy[ci] > 0)
					dd_srd_load_ci(srd, &ni, &nr, &nd, ci, flags);
			}
		}
		break;
	case YY:
...
static gmx_inline void dd_srd_send(
		t_srd*			srd,
		gmx_domdec_t*	dd,
		int		ddimind,
		int		direction,
...
{
	int nrecv;
	int ioffset = srd->irecv;
	int roffset = srd->rrecv;
	int doffset = srd->drecv;

	dd_sendrecv_int(dd, ddimind, direction, &srd->isend, 1, &srd->irecv, 1);
	realloc_srd_int_buffer(srd, srd->rbuf_int, srd->irecv + ioffset);

	dd_sendrecv_int(dd, ddimind, direction,
		srd->sbuf_int->buf, srd->isend,
		srd->rbuf_int->buf + ioffset, srd->irecv);

	if (flags & SRD_COMM_FLAGS_REAL)
	{
		dd_sendrecv_int(dd, ddimind, direction, &srd->rsend, 1, &srd->rrecv, 1);
		realloc_srd_real_buffer(srd, srd->rbuf_real, srd->rrecv + roffset);

		dd_sendrecv_real(dd, ddimind, direction,
			srd->sbuf_real->buf, srd->rsend,
			srd->rbuf_real->buf + roffset, srd->rrecv);
	}

	if (flags & SRD_COMM_FLAGS_DOUBLE)
	{
		dd_sendrecv_int(dd, ddimind, direction, &srd->dsend, 1, &srd->drecv, 1);
		realloc_srd_double_buffer(srd, srd->rbuf_double, srd->drecv + doffset);

		dd_sendrecv_double(dd, ddimind, direction,
			srd->sbuf_double->buf, srd->dsend,
			srd->rbuf_double->buf + doffset, srd->drecv);
	}
}

void dd_srd_comm(t_srd* srd, gmx_domdec_t* dd, int flags)
{
	int nsend, nrecvBackward, nrecvForward;

	int d;
	int ddimind = 0;
	for (d = 0; d < DIM; d++)
	{
		// skip dimensions with only one dd zone
		if (dd->nc[d] < 2)
			continue;
		...
		// backward communication pulse
		nsend = dd_srd_load(srd, d, srd->sharedCellCountB, srd->sharedCellsB, flags);
		dd_sendrecv_int(dd, ddimind, dddirBackward, &nsend, 1, &nrecvBackward, 1);
		dd_srd_send(srd, dd, ddimind, dddirBackward, flags);

		// forward communication pulse
		nsend = dd_srd_load(srd, d, srd->sharedCellCountF, srd->sharedCellsF, flags);
		dd_sendrecv_int(dd, ddimind, dddirForward, &nsend, 1, &nrecvForward, 1);
		dd_srd_send(srd, dd, ddimind, dddirForward, flags);

		dd_srd_accumulate(srd, nrecvBackward + nrecvForward, flags);
		ddimind++;
	}
}
