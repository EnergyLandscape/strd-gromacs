...
#pragma omp parallel for num_threads(nthread) schedule(static)
    ...
    {
        calc_column_indices(grid, a0, a1, particlePositionArray, dd_zone, move, thread, nthread,
...
#pragma omp parallel for num_threads(nthread) schedule(static)
    for (thread = 0; thread < nthread; thread++)
    {
        if (grid->bSimple)
        {
            sort_columns_simple(nbs, dd_zone, grid, a0, a1, atinfo, particlePositionArray, nbat,
        ...
        {
            sort_columns_supersub(nbs, dd_zone, grid, a0, a1, atinfo, particlePositionArray, nbat,
                                  ((thread+0)*grid->ncx*grid->ncy)/nthread,
                                  ((thread+1)*grid->ncx*grid->ncy)/nthread,
                                  nbs->work[thread].sort_work);
        }
                       ...
                       nbnxn_atomdata_t *nbat)
{
    ...
    calc_cell_indices(nbs, dd_zone, grid, a0, a1, atinfo, particlePositionArray, move, nbat);
