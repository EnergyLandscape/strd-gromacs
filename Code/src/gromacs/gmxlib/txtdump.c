		...
		gmx_bool bMDPformat)
{
	...
	if (available(fp, ir, indent, title))
	{
		...
		// ===================================================================
		//SRDCODE added MPC paramters to gmxdump
		PS("mpc-type", EMPC(ir->empcType));
		PI("mpc-seed", ir->mpcSeed);
		PI("mpc-freq", ir->mpcCollisionFrequency);
		PR("mpc-cellsize", ir->mpcCellSize);
		PR("srd-angle", ir->srdAngle);
		PS("srd-tcouple", EBOOL(ir->tcoupleSRD));
		PR("srd-tc-str", ir->srdThermostatStrength);
		PR("srd-tc-ref-t", ir->srdThermostatTarget);
		PI("mpc-out-freq", ir->mpcOutputFrequency);
		PI("mpc-out-samp", ir->mpcOutputSampling);
		PS("mpc-out-vel", EBOOL(ir->mpcWriteCellVectors));
		PS("mpc-out-num", EBOOL(ir->mpcWriteCellOccupancy));
		//END_SRDCODE
		// ===================================================================
