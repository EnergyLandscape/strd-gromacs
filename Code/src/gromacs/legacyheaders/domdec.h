...
// ===================================================================
//SRDCODE function prototypes for new communication in domdec.c

#ifdef SRD_ROUTING_DEBUG
void get_reference_cell_homes(t_srd* srd, gmx_domdec_t *dd, matrix box);
/* Assigns srd->cellHomeRanks for testing the routing system */

void printCell(gmx_int64_t step, t_commrec* cr);
/* Prints the boundaries of the calling processor's domain */

void printAllCells(gmx_int64_t step, gmx_domdec_t *dd);
/* Prints all the domain decomposition boundaries */
#endif

void get_dd_bounds(t_commrec* cr, rvec x0, rvec x1);
/* Gets local DD boundaries */

void get_neighboring_boundaries(t_srd* srd, t_commrec* cr);
/* Gets current DD boundaries from neighbors */

void get_neighboring_cell_range(t_srd* srd, t_commrec* cr);
/* Gets current collision cell range from DD neighbors */

void dd_srd_init(t_srd* srd, t_commrec* cr);
/* Prepares the routing map for SRD communication */

void dd_srd_update(t_srd* srd, t_commrec* cr);

void dd_before_srd_collisions(t_srd* srd, t_state* state, t_commrec* cr);
/* SRD communication before the collision step */

void dd_after_srd_collisions(t_srd* srd, t_state* state, t_commrec* cr);
/* SRD communication following the collision step */


// new
void dd_srd_find_local_collision_cells(t_srd* srd, t_commrec* cr);
void dd_srd_comm(t_srd* srd, gmx_domdec_t* dd, int phase);
