...
typedef struct {
    ...
    // ===================================================================
    //SRDCODE new input parameters for MPC
    int				empcType;				/* Run SRD dynamics? */
    gmx_int64_t		mpcSeed;				/* seed for the RNG used by MPC */
    int				mpcCollisionFrequency;	/* Interval for collision steps, in simulation steps. */
    real			mpcCellSize;			/* Preferred cell size, code will choose closest. */
    ivec			mpcInitialCellCount;	/* Initial cell count, as determined by grompp */
    real			srdAngle;				/* Rotation angle for collision step. */
    gmx_bool		tcoupleSRD;				/* Use cell-level SRD thermostat? */
    real			srdThermostatStrength;	/* Controls size of fluctuations used to drive system toward reference t */
    real			srdThermostatTarget;	/* Reference temperature for SRD thermostat */
    int				mpcOutputFrequency;		/* How often to write to the output file -- value of zero indicates no output */
    int				mpcOutputSampling;		/* How often to sample cell data for outputting -- these values will be averaged */
    gmx_bool		mpcWriteCellVectors;	/* Write cell velocity vectors */
    gmx_bool		mpcWriteCellOccupancy;	/* Write cell occupancy numbers */
    //END_SRDCODE
    // ===================================================================
