...
#define NFILE asize(fnm)
    ...
    // ===================================================================
    //SRDCODE check for bad input combinations do some initialization

   	if(ir->empcType != empcOFF)
   	{
   		if (ir->eI != eiVV)
   			gmx_fatal(FARGS, "MPC can only be used with the velocity-verlet integrator.");

		if (ir->mpcOutputFrequency > 0)
		{
			if (ir->mpcOutputFrequency < ir->mpcCollisionFrequency)
			{
				sprintf(warn_buf, "<MPC> Ouput frequency less than collision frequency! Setting equal to collision frequency.\n");
				warning_error(wi, warn_buf);
				ir->mpcOutputFrequency = ir->mpcCollisionFrequency;
			}

			if (ir->mpcOutputFrequency % ir->mpcCollisionFrequency != 0)
			{
				int nearest = ir->mpcCollisionFrequency * (ir->mpcOutputFrequency / ir->mpcCollisionFrequency);
				sprintf(warn_buf, "<MPC> Output frequency not an even multiple of collision frequency! Setting to nearest value: %d\n", nearest);
				warning_error(wi, warn_buf);
				ir->mpcOutputFrequency = nearest;
			}

			if (ir->mpcOutputSampling == 0)
			{
				sprintf(warn_buf, "<MPC> Output sampling set to zero. Setting equal to output frequency.\n");
				warning_error(wi, warn_buf);
				ir->mpcOutputSampling = ir->mpcOutputFrequency;
			}

			if (ir->mpcOutputSampling < ir->mpcCollisionFrequency)
			{
				sprintf(warn_buf, "<MPC> Output sampling less than collision frequency! Setting equal to collision frequency.\n");
				warning_error(wi, warn_buf);
				ir->mpcOutputSampling = ir->mpcCollisionFrequency;
			}

			if (ir->mpcOutputSampling % ir->mpcCollisionFrequency != 0)
			{
				int nearest = ir->mpcCollisionFrequency * (ir->mpcOutputSampling / ir->mpcCollisionFrequency);
				sprintf(warn_buf, "<MPC> Output sampling not an even multiple of collision frequency! Setting to nearest value: %d\n", nearest);
				warning_error(wi, warn_buf);
				ir->mpcOutputSampling = nearest;
			}

			if (ir->mpcOutputFrequency % ir->mpcOutputSampling != 0)
			{
				// don't try to fix this one
				gmx_fatal(FARGS, "<MPC> Output frequency is not an even multiple of output sampling.\n");
			}
		}

		if (ir->mpcSeed == -1)
		{
			ir->mpcSeed = (gmx_int64_t)gmx_rng_make_seed();
			fprintf(stderr, "<MPC> Setting the MPC random seed to %"GMX_PRId64"\n", ir->mpcSeed);
		}

		int d;
		for (d = 0; d < DIM; d++)
		{
			int n = (int)round(state.box[d][d] / ir->mpcCellSize);
			n = max(1, n);
			ir->mpcInitialCellCount[d] = n;
		}

		fprintf(stderr, "<MPC> Initial cell grid is %d x %d x %d\n",
			ir->mpcInitialCellCount[XX], ir->mpcInitialCellCount[YY] , ir->mpcInitialCellCount[ZZ]);
   	}
