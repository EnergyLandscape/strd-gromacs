            ...
            warninp_t wi)
{
    ...
    // ===================================================================
    //SRDCODE new input parameters for grompp

    CCTYPE ("MPC OPTIONS");
    CTYPE ("MPC algorithm (off/srd/at/at+a)");
    EETYPE("mpc-type",	   ir->empcType, empc_names);
    STYPE ("mpc-grps",	   is->mpc_grps, NULL);
    CTYPE ("number of timesteps between each collision");
    ITYPE ("mpc-freq",     ir->mpcCollisionFrequency, 16);
    CTYPE ("preferred cubic cell size");
    RTYPE ("mpc-cellsize", ir->mpcCellSize, 1.0f);
    CTYPE ("RNG seed used by MPC");
    STEPTYPE ("mpc-seed",  ir->mpcSeed, -1);

    CCTYPE ("SRD OPTIONS");
    CTYPE ("rotation angle used in SRD collisions");
    RTYPE ("srd-angle",    ir->srdAngle, 15.0f);
    // srd thermostat
    CTYPE ("enable cell-level thermostat for SRD particles (yes/no)");
    EETYPE("srd-tcouple",  ir->tcoupleSRD, yesno_names);
    CTYPE ("srd thermostat strength (~0.05 to ~0.30) and target temperature");
    RTYPE ("srd-tc-str",   ir->srdThermostatStrength, 0.10f);
    RTYPE ("srd-tc-ref-t", ir->srdThermostatTarget, 310.0f);

    CCTYPE ("MPC OUTPUT");
    CTYPE ("number of collision steps between output frames (0 = no output)");
    ITYPE ("mpc-out-freq", ir->mpcOutputFrequency, 0);
    ITYPE ("mpc-out-samp", ir->mpcOutputSampling, 0);
    CTYPE ("write MPC cell velocities and occupancies (yes/no)");
    EETYPE("mpc-out-vel",  ir->mpcWriteCellVectors, yesno_names);
    EETYPE("mpc-out-num",  ir->mpcWriteCellOccupancy, yesno_names);
              ...
              warninp_t wi)
{
    ...
    // ===================================================================
    //SRDCODE read MPC groups

	int nSRD = str_nelem(is->mpc_grps, MAXPTR, ptr1);
	do_numbering(natoms, groups, nSRD, ptr1, grps, gnames, egcMPC,
