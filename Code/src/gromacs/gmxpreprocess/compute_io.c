                  ...
                  int nrener, int nrepl)
{
    ...
    // ===================================================================
    //SRDCODE account for size of binary output file
    if (ir->empcType != empcOFF && ir->mpcOutputFrequency > 0)
    {
    	int numCells = ir->mpcInitialCellCount[XX]*ir->mpcInitialCellCount[YY]*ir->mpcInitialCellCount[ZZ];
    	...
    	// frame header
    	cio += div_nsteps(nsteps, ir->mpcOutputFrequency)*sizeof(int);
    	cio += div_nsteps(nsteps, ir->mpcOutputFrequency)*sizeof(float);
    	cio += div_nsteps(nsteps, ir->mpcOutputFrequency)*2*sizeof(rvec);
    	// cell data
    	if (ir->mpcWriteCellOccupancy)
    		cio += div_nsteps(nsteps, ir->mpcOutputFrequency)*numCells*sizeof(real);
    	if (ir->mpcWriteCellVectors)
    		cio += div_nsteps(nsteps, ir->mpcOutputFrequency)*numCells*sizeof(rvec);
    }
	//END_SRDCODE
    // ===================================================================
