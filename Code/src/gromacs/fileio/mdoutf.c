		...
		rvec *f_local, rvec *f_global)
{
	...
	if (MASTER(cr))
	{
		...
		// This code was modified to use the compressed-x-grps parameter with TRR files.
		// If there are xtc groups present, only the state of those atoms will be written
		// to the TRR file. The approach closely mirrors the one taken by the XTC writing
		// code: copy the parameters into temporary arrays and pass those coordinates to
		// the fwrite_trn(...) and gmx_fwrite_tng(...) functions.
		if (mdof_flags & (MDOF_X | MDOF_V | MDOF_F))
		{
			...
			rvec *ff = NULL;
			int natoms;
			gmx_bool writeSubset = (of->natoms_x_compressed != of->natoms_global);

			if (writeSubset)
			{
				int i, j;

				if(mdof_flags & MDOF_X)
					snew(xx, of->natoms_x_compressed);
				if(mdof_flags & MDOF_V)
					snew(vv, of->natoms_x_compressed);
				if(mdof_flags & MDOF_F)
					snew(ff, of->natoms_x_compressed);
				...
				for (i = 0, j = 0; (i < of->natoms_global); i++)
				{
					...
					{
						if(mdof_flags & MDOF_X)
							copy_rvec(state_global->x[i], xx[j]);
						if(mdof_flags & MDOF_V)
							copy_rvec(global_v[i], vv[j]);
						if(mdof_flags & MDOF_F)
							copy_rvec(f_global[i], ff[j]);
						j++;
					}
				}

				natoms = of->natoms_x_compressed;
			}
			else
			{
				if(mdof_flags & MDOF_X)
			...
			if (of->fp_trn)
			{
				fwrite_trn(of->fp_trn, step, t, state_local->lambda[efptFEP],
						state_local->box, natoms, xx, vv, ff);
			...
			gmx_fwrite_tng(of->tng, FALSE, step, t, state_local->lambda[efptFEP],
					...
					natoms, (const rvec *) xx, (const rvec *) vv, (const rvec *) ff);
			if(writeSubset)
			{
				if(mdof_flags & MDOF_X)
					sfree(xx);
