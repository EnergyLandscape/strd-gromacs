 ...
 * file_version. */
//SRDCODE extended this enum to increment tpx file version
enum tpxv {
...
//SRDCODE use the new tpx file version
static const int tpx_version = tpxv_MultiParticleCollisions;
		...
		int file_version, real *fudgeQQ)
{
	...
	//SRDCODE: read/write srd inputrec parameters to tpr file
	if (file_version >= 80)
	{
		gmx_fio_do_int(fio, ir->empcType);
		if(ir->empcType != empcOFF)
		{
			gmx_fio_do_int64(fio,  ir->mpcSeed);
			gmx_fio_do_int(fio,  ir->mpcCollisionFrequency);
			gmx_fio_do_real(fio, ir->mpcCellSize);
			gmx_fio_do_ivec(fio, ir->mpcInitialCellCount);
			gmx_fio_do_real(fio, ir->srdAngle);
			gmx_fio_do_gmx_bool(fio, ir->tcoupleSRD);
			if(ir->tcoupleSRD)
			{
				gmx_fio_do_real(fio, ir->srdThermostatStrength);
				gmx_fio_do_real(fio, ir->srdThermostatTarget);
			}
			gmx_fio_do_int(fio, ir->mpcOutputFrequency);
			if(ir->mpcOutputFrequency > 0)
			{
				gmx_fio_do_int(fio, ir->mpcOutputSampling);
				gmx_fio_do_gmx_bool(fio, ir->mpcWriteCellVectors);
				gmx_fio_do_gmx_bool(fio, ir->mpcWriteCellOccupancy);
			}
		}
	}
}
