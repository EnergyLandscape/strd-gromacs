		...
		gmx_walltime_accounting_t walltime_accounting)
{
	...
	//SRDCODE domain decomposition has been initialized, time to initialize SRD
	t_srd* srd = (t_srd*)malloc(sizeof(t_srd));
	init_srd(fplog, srd, state, state_global, top_global, ir, cr);
	//END_SRDCODE
	...
	while (!bLastStep || (bRerunMD && bNotLastFrame))
	{
		...
		while (bFirstIterate || iterate.bIterationActive)
		{
			...
			if (!(bRerunMD && !rerun_fr.bV && !bForceUpdate))
			{
				...
				// ===================================================================
				//SRDCODE update on-step velocity v_n using on-step position x_n
				if(bVV && ir->empcType != empcOFF)
				{
					wallcycle_stop(wcycle, ewcUPDATE);
					update_srd(srd, step, state, state_global, mdatoms, cr, wcycle);
